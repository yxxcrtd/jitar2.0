...
import cn.edustar.jitar.converter.DocConverter;
import cn.edustar.jitar.converter.JacobPDFConverter;
import cn.edustar.jitar.converter.SWFToolsSWFConverter;
		...
		if (cmd.equals("uploadpreparecourse")) {
			...
			if(ownerFlag.equals("0"))
			{
				if (!contentType.equalsIgnoreCase("application/msword")	&& !contentType.equalsIgnoreCase("application/vnd.openxmlformats-officedocument.wordprocessingml.document")) {
					...
					response.setContentType("text/plain;charset=UTF-8");
					response.setStatus(HttpServletResponse.SC_OK);
					response.getWriter().write("上传的文档类型不正确，必须是 word 文档。");
					return NONE;
				}
			}
