...
public class ShowResourceAction extends ManageBaseAction{
	...
	private void showResourceDetail(){
        ...
        if (resourceFolder  == null || resourceFolder.trim().length() == 0){
            setRequestAttribute("error", "资源根文件夹路径不存在。");
            return ;
        }
        
        if (filename  == null || filename.trim().length() == 0){
            setRequestAttribute("error", "数据库中的资源路径不存在。");
            return;
        }        
        ...
        if(!fs.isFile() && !fs.exists()){
            setRequestAttribute("error", "资源的真实路径不存在。");
            fs = null;
            return;
        }
        fs = null;
	    ...
	    try{
            if(filename.endsWith(".jpg") || filename.endsWith(".gif") || filename.endsWith(".png")){
                ...
                {
                    setRequestAttribute("error", "本身是Flash文件，但文件不存在。");
                    return;
                }                
                ...
                {
                    setRequestAttribute("error", "swf 文件不存在：<br/><br/>可能文件未转换成功，或者还未进行转换。");
                    return;
                } 
            }
	    }catch(Exception ex){
