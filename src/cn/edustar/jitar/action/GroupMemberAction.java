...
public class GroupMemberAction extends BaseGroupAction {
	...
	private String invite_user() {
		...
		if (user == null) {
			addActionError("未找到登录名为 " + loginName + " 的用户.");
			return ERROR;
		}

		// 查看用户是否已经是成员了.
		GroupMember gm = group_svc.getGroupMemberByGroupIdAndUserId(group_model.getGroupId(), user.getUserId());
		if (gm != null) {
			if (gm.getStatus() != GroupMember.STATUS_NORMAL)
				addActionError("该用户已经是协作组成员, 但未经审核、锁定、删除或未接受邀请.");
			else
				addActionError("该用户已经是协作组成员了.");
			return ERROR;
		}
		...
		addActionMessage("已经向用户 " + user.toDisplayString() + " 发出了邀请.");
		return SUCCESS;
	}
