...
public class SiteUnitsAction extends AbstractBasePageAction {
    ...
    public String execute(String cmd) throws Exception {
        ...
        if (ConfigUnitLevel == 1) {
            ...
            List<Unit> childUnitList = unitService.getChildUnitListByParenId(pid);
            ...
            List<Unit> childUnitList = unitService.getChildUnitListByParenId(rootId);
            List<JSONObject> parentUnitArray = new ArrayList<JSONObject>();
            for (Unit cunit : childUnitList) {
                List<Unit> ccunitList = unitService.getChildUnitListByParenId(cunit.getUnitId());
    ...
    private void get_unit_tree() {
        ...
        Unit rootUnit;
        if (null == html) {
            List<Unit> u_list = unitService.getChildUnitListByParenId(0);
            if (u_list.size() < 1) {
                rootUnit = null;
            } else {
    ...
    private void getUnitDataList() throws IOException{
        ...
        List<Unit> unitlist = unitService.getChildUnitListByParenId(pid);
        String json = "[";
