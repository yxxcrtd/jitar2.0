...
/**
 *  协作组查询
 *  将 py中的 GroupQuery  修改为 java的 GroupQuery
 *  
 * @author baimindong
 *
 */
	  ...
	  public  GroupQuery(String selectFields) {
	    ...
	    public void applyWhereCondition(QueryContext qctx) {
            ...
            if(gradeId != null && subjectId != null){
	              ...
	              qctx.addAndWhere("g.XKXDId LIKE :likeKey");
	              qctx.setString("likeKey", "%," + gradeId +"/%");
            }else if(subjectId != null){
	              qctx.addAndWhere("g.XKXDId LIKE :likeKey");
	              qctx.setString("likeKey", "%/" + subjectId +",%");
            }
	        if(parentId!=null){
	        ...
	        if(k != null && k.length() > 0){
	            ...
	            qctx.addAndWhere("(g.groupName LIKE :likeKey) OR (g.groupTitle LIKE :likeKey) OR (g.groupTags LIKE :likeKey) OR (g.groupIntroduce LIKE :likeKey)");
