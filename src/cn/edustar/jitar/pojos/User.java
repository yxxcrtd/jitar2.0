...
/**
 * User 表示群组系统中用户对象. 以'loginName'做为业务键.
 * 
 *
 */
public class User implements Serializable, Cloneable, UserConst {
	
	/** serialVersionUID */
	private static final long serialVersionUID = 2966959632081106262L;

	/** 用户登录之后在 session 中保存 UserModel 对象的键名字, 在模板中通过 ${loginUser} 即可访问 */
	public static final String SESSION_LOGIN_USERMODEL_KEY = "loginUser";

	/** 从统一用户中获取的用户对象模型, 其中包括当前用户的登录次数等信息, 在模板中通过 ${__genuser} 访问 */
	public static final String SESSION_LOGIN_GENUSER_KEY = "__genuser";
	
	/** 用户登录之后在 session 中保存 'loginName' 的键名字 */
	public static final String SESSION_LOGIN_NAME_KEY = "jitar.login.loginName";

	/** 用户票证在 session 中保存的键的名字 */
	public static final String SESSION_USER_TICKET = "jitar.login.userTicket";
	...
	/**
	 * 清除在 session 中记录的和用户有关的所有键
	 * 
	 * @param session
	 */
	public static final void clearLoginSession(HttpSession session) {
		if (session == null) return;		
		session.removeAttribute(SESSION_LOGIN_USERMODEL_KEY);
		session.removeAttribute(SESSION_LOGIN_GENUSER_KEY);
		session.removeAttribute(SESSION_LOGIN_NAME_KEY);
		session.removeAttribute(SESSION_USER_TICKET);
	}
	 ...
	 * 则必须如 'http://domain/...' 格式.
	 */
	private String userIcon = DEFAULT_USER_ICON;   //只保存文件名
	...
	public boolean equals(Object o) {
		...
		return PojoHelper.equals(this.loginName, ((User) o).loginName);
	}
