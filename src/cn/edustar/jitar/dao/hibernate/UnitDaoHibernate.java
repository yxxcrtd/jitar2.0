...
public class UnitDaoHibernate extends BaseDaoHibernate implements UnitDao {
	...
	@SuppressWarnings("unchecked")
	public List<Unit> getAllUnitOrChildUnitList(Unit unit) {
		String queryString;
		if (null == unit) {
	...
	@SuppressWarnings("unchecked")
	public List<Unit> getChildUnitListByParenId(int parentId) {
		String queryString;
		queryString = "From Unit Where parentId = :parentId Order By itemIndex ASC";
	...
	public Long getAllUserCount(Unit unit){
		...
		String queryString = "SELECT SUM(userCount) FROM Unit WHERE unitPathInfo LIKE '%/" + unit.getUnitId() + "/%'";
	...
	public Long getAllArticleCount(Unit unit){
		if(unit == null) return 0L;
		String queryString = "SELECT SUM(articleCount) + SUM(historyArticleCount) FROM Unit WHERE unitPathInfo LIKE '%/" + unit.getUnitId() + "/%'";
	...
	public Long getAllResourceCount(Unit unit){
		if(unit == null) return 0L;
		String queryString = "SELECT SUM(resourceCount) FROM Unit WHERE unitPathInfo LIKE '%/" + unit.getUnitId() + "/%'";
		Object count = this.getSession().createQuery(queryString).uniqueResult();
		if(count == null)
			return 0L;
		else
	...
	public Long getAllPhotoCount(Unit unit){
		if(unit == null) return 0L;
		String queryString = "SELECT SUM(photoCount) FROM Unit WHERE unitPathInfo LIKE '%/" + unit.getUnitId() + "/%'";
		Object count = this.getSession().createQuery(queryString).uniqueResult();
		if(count == null)
			return 0L;
		else
	...
	{
		if(unit == null) return 0L;
		String queryString = "SELECT SUM(videoCount) FROM Unit WHERE unitPathInfo LIKE '%/" + unit.getUnitId() + "/%'";
		Object count = this.getSession().createQuery(queryString).uniqueResult();
		if(count == null)
			return 0L;
		else
