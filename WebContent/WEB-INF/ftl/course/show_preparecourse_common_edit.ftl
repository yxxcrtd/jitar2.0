...
<body<#if prepareCourse.contentType != 2 && prepareCourse.contentType != 3 && prepareCourse.contentType != 4 && prepareCourse.contentType != 100> onbeforeunload='return returnCheck(event)'</#if>>
	...
	<#elseif prepareCourse.contentType == 100 >
		...
		<div style="padding:4px 0">先下载 <a target="_blank" href="${SiteUrl}manage/DownloadCourseFile?p=${prepareCourse.prepareCourseId}&t=doc&f=1">下载 Word 格式</a> 文件，编辑完成后再这里上传，或者“取消上传并解除锁定”。</div>
		</#if>
	...
	<div style="padding-top:6px;color:#f00">说明1：Word文档可以是<b>Word 2003、Word 2007、Word 2010 、Word 2013</b>版本生成的文件；<br/>
    ...
    <#else>    
       ...
       <form method='post' id='ff'>
		...
		<input type="hidden" id='commonContent' name="commonContent" value="${prepareCourse.commonContent!?html}" />     
		<iframe id="DHtml_Frame" src="${SiteUrl}manage/fckeditor/editor/fckeditor.jsp?InstanceName=commonContent&Toolbar=Default" width="100%" height="800" frameborder="0" scrolling="no"></iframe>
