...
<td id='column_2' class='col'>
	...
	<div id='placerholder1' title='查看个案内容' style='padding:1px;'>
		...
		<#else>
			...
			<#else>
			<#if loginUser??>
			<div style="text-align:right;padding:4px 0"><a target="_blank" href="${SiteUrl}manage/DownloadCourseFile?p=${prepareCourseMember.prepareCourseMemberId}&t=doc&f=0">下载 Word 格式</a> <a target="_blank" href="${SiteUrl}manage/DownloadCourseFile?p=${prepareCourseMember.prepareCourseMemberId}&t=pdf&f=0">下载 PDF 格式</a> </div>
			</#if>
