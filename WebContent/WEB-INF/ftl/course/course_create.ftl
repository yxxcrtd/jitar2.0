...
<form method="post" name='pc'<#if prepareCourse??> action='createPreCourse.py?prepareCourseId=${prepareCourse.prepareCourseId!0}'</#if>>
...
<#if prepareCourse??>
<input name="pcStartDate" id='start-date' value='${prepareCourse.startDate?string('yyyy-MM-dd')}' class="date-pick" />
<#else>
...
<#if prepareCourse??>
<input name="pcEndDate" id='end-date' value='${prepareCourse.endDate?string('yyyy-MM-dd')}' class="date-pick" />
<#else>
...
<#if prepareCourse??>
<input type="hidden" id="DHtml" name="pcDescription" value="${prepareCourse.description?html}" />     
<#else>
<input type="hidden" id="DHtml" name="pcDescription" value="" /> 
</#if>
...
<#else>
<input type='button' value=' 返  回 ' onclick='window.location="../user_manage.action"' />
</#if>
</td>
</tr>
</table>
