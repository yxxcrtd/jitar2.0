...
<form name='newsForm' id='newsForm'  action='admin_news.action' method='post'>
	...
	<input type="hidden" name="type" value="${newsType!}" />
	  <div class='pager'>
		  <a href='?cmd=list&amp;subjectId=-1'>所有${displayName!}</a> |
		  <a href='?cmd=list&amp;subjectId=0'>主页${displayName!}</a> 
		  ...
		  <#list subject_list as s>			
		    <option value='${s.subjectId}' >${displayName!}</option>
		  </#list>
		  </#if>
		  </select>
	  </div>
...
<table class='listTable' cellspacing='1'>
  ...
  <#list news_list as news>
    <tr>
      ...
      <td>
        <a href='?cmd=edit&amp;newsId=${news.newsId}&type=${newsType!}'>修改</a>
        <#if news.status == 1><a href='?cmd=audit&amp;newsId=${news.newsId}&type=${newsType!}'>审核</a></#if>
        <a href='?cmd=delete&amp;newsId=${news.newsId}&type=${newsType!}' onclick='return confirm("您真的要删除吗？")'>删除</a>
      </td>
    </tr>
  </#list>
  </#if>
