...
<tr>
  ...
  <#list specialsubject_list as sl>
    ...
    <tr style="display:<#if showinput==1><#else>none</#if>;" id="inputDiv">
     <td align="right"><b>文章内容：</b></td>
     <td>              
      <textarea name="articleContent" id="HtmlEditor" style="display: none;">${article.articleContent!?html}</textarea>
      <input type="hidden" name="userid" id="userid" value="${userid!?html}"/>
      <iframe id="mainframe" src="${SiteUrl}manage/fckeditor/editor/fckeditor.jsp?InstanceName=HtmlEditor&Toolbar=Default" width="100%" height="400" frameborder="0" scrolling="no"></iframe>
    </td>
</tr>
<tr>
