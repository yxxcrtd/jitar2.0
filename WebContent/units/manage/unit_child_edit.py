...
class unit_child_edit(ActionExecutor):
    ...
    def execute(self):
        parentId = self.params.safeGetIntParam("unitId")
        unitId = self.params.safeGetIntParam("childUnitId")
        parentUnit = self.unitService.getUnitById(parentId)
        if parentUnit == None:
            self.addActionError(u"无效的父节点。")
            self.addActionLink(u"返回", "unit_child.py?unitId=" + str(parentId))
        ...
        if request.getMethod() == "POST":
            ...
            if enname == "" or title == "":
                ...
                self.addActionLink(u"返回", "unit_child_edit.py?unitId=" + str(parentId) + "&childUnitId=" + str(unitId))
                return ActionResult.ERROR
            if siteTitle == "":
            ...
            unit.setUnitPathInfo(parentUnit.unitPathInfo + str(unit.unitId) + "/")
            self.unitService.saveOrUpdateUnit(unit)
            ...
            self.addActionLink(u"返回列表", "unit_child.py?unitId=" + str(parentId))
            return ActionResult.SUCCESS
        
        request.setAttribute("parentUnit", parentUnit)
        request.setAttribute("unit", unit)
        return "/WEB-INF/unitsmanage/unit_child_edit.ftl"
